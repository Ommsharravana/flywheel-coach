...
export default function PublicProblemsPage() {
  ...
  const fetchProblems = useCallback(async () => {
    ...
    try {
      ...
      const uniqueInstitutions = new Set(data.data.map((p: ProblemCardData) => p.institution_short).filter(Boolean));
      setStats({
        total: data.total,
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          // Redirect to login
          router.push('/login?redirect=/appathon/problems');
          return;
        }
        ...
        <div className="text-center mb-12">
          <div className="flex items-center justify-center gap-2 mb-4">
            ...
            <h1 className="text-4xl font-display font-bold text-stone-100">
              Appathon 2.0 Problem Bank
            </h1>
          </div>
          <p className="text-stone-400 text-lg max-w-2xl mx-auto">
            Browse real problems from JKKN institutions. Find one that inspires you,
            fork it to your own cycle, and build a solution that makes a difference.
          </p>
        </div>
    ...
    <Card className="bg-stone-900/50 border-stone-800 hover:border-stone-700 transition-all hover:shadow-lg hover:shadow-amber-500/5">
      <CardContent className="pt-6">
        ...
        <div className="flex items-center justify-between pt-3 border-t border-stone-800">
          ...
          <div className="flex items-center gap-2">
            <Link href={`/appathon/problems/${problem.id}`}>

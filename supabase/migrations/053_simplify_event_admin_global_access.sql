-- Migration: Simplify Event Admin to Global Access
--
-- The user requested to simplify the permission model:
-- - role='event_admin' now sees ALL users/events (global access, like superadmin)
-- - Remove all event_admins table lookups in RPC functions
-- - The event_admins table is kept for backward compatibility but no longer used for scoping
--
-- APPLIED: 2025-12-29

-- =============================================================================
-- FIX: get_all_users_admin - event_admin sees ALL users (global access)
-- =============================================================================
...
BEGIN
  ...
  -- Event admin sees ALL users (global access - simplified model)
...
-- =============================================================================
-- FIX: get_all_problems_admin - event_admin sees ALL problems (global access)
-- =============================================================================
...
BEGIN
  ...
  -- Event admin sees ALL problems (global access - simplified model)
  ELSIF calling_user_role = 'event_admin' THEN
    RETURN QUERY
    SELECT
      p.id,
...
-- =============================================================================
-- FIX: get_eligible_cycles_admin - event_admin sees ALL cycles (global access)
-- =============================================================================
...
BEGIN
  ...
  -- Event admin sees ALL cycles (global access - simplified model)
  ELSIF calling_user_role = 'event_admin' THEN
    RETURN QUERY
    SELECT
      c.id,
...
-- =============================================================================
-- FIX: get_admin_stats - event_admin sees ALL stats (global access)
-- =============================================================================
...
BEGIN
  ...
  -- Superadmin and event_admin see global stats
  IF calling_user_role IN ('superadmin', 'event_admin') THEN

-- Fix: Restore institution columns to get_eligible_cycles_admin
-- Bug: BUG-103 - Institution breakup is inaccurate on Problem Bank page
-- Root Cause: Migration 054 removed institution_id, institution_name, institution_short_name
--             from the function return type and removed the institutions join
-- Fix: Restore the original function signature with institution data

-- Drop the incorrect function first (it has different signature)
DROP FUNCTION IF EXISTS get_eligible_cycles_admin(UUID, INT, INT);

-- Restore function with correct signature and institution columns

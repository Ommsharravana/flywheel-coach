-- Fix event_admin access to Problem Bank
-- Event admins need to see cycles from their events
...
BEGIN
  ...
  -- FIX: Allow event_admin role as well
  IF calling_user_role NOT IN ('superadmin', 'institution_admin', 'event_admin') THEN
    RETURN;
  END IF;
  ...
  -- Institution admin and Event admin see only their event's cycles
  ELSE
    RETURN QUERY
    SELECT
      c.id,
